...
const sheetVariants = cva(
  "fixed z-50 gap-4 overflow-y-auto overflow-x-hidden border-border bg-background p-6 shadow-lg transition ease-in-out data-[state=closed]:animate-out data-[state=open]:animate-in data-[state=closed]:duration-300 data-[state=open]:duration-500",
  {
    variants: {
      side: {
...
export const SheetHeader = ({ className, ...props }: SheetHeaderProps) => (
  <div
    className={cn(
      "flex flex-col space-y-2 text-center font-medium text-lg sm:text-left",
      className,
    )}
    {...props}
  />
...
export const SheetTitle = ({ className, ...props }: SheetTitleProps) => (
  <SheetPrimitive.Title
    className={cn("font-semibold text-2xl text-primary-dimmed", className)}
    {...props}
  />
);
